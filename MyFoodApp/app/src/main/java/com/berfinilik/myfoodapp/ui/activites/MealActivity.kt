...
import com.berfinilik.myfoodapp.adapters.SetOnMealClickListener
import com.berfinilik.myfoodapp.data.pojo.Meal
import com.berfinilik.myfoodapp.mvvm.MealActivityMVVM
import com.berfinilik.myfoodapp.R
import com.berfinilik.myfoodapp.databinding.ActivityCategoriesBinding
...
class MealActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mealActivityMvvm = ViewModelProvider(this).get(MealActivityMVVM::class.java)
        startLoading()
        prepareRecyclerView()
        mealActivityMvvm.getMealsByCategory(getCategory())
        mealActivityMvvm.observeMeal().observe(this, Observer { t ->
            if (t == null) {
                hideLoading()
                Toast.makeText(applicationContext, "Bu kategoride yemek yok", Toast.LENGTH_SHORT).show()
                onBackPressed()
            } else {
